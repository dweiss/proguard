...
public class ConfigurationParser implements AutoCloseable
{
    ...
    private List<PackageRenameRule> parsePackageRenameRule(List<PackageRenameRule> list) throws IOException, ParseException
    {
        ...
        readNextWord("package rename pattern");
        Pattern pattern;
        try
        {
            pattern = Pattern.compile(nextWord);
        }
        catch (PatternSyntaxException e)
        {
            throw new ParseException("Could not parse regexp pattern for package matcher: " + nextWord + ", cause: " + e.getMessage());
        }

        readNextWord("=> literal");
        if (!nextWord.equals("=>"))
        {
            throw new ParseException("Expected '=>' literal: " + nextWord);
        }

        readNextWord("package name replacement regexp string");
        String replacement = nextWord;
